function [vec, residNorm_prev, iter, time_sum, ksum] = CGIHT_M_dct (y, dct_rows, k, m, n, tol, maxiter, restartFlag)
...
grad_previous = AT_dct(resid, m, n, dct_rows);
grad=grad_previous;
grad_prev_thresh = threshold_one(grad_previous, bin, kbin);

% check for converdct ce
...
while ( (err>resid_tol) && (iter < maxiter) && (err < 100 * err_start) && (abs(residNorm_diff) > .01*tol) && (fail == 0) )
    ...
    if (restartFlag)
      [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = RestrictedCGwithSupportEvolution_M_dct(vec, grad, grad_previous, grad_prev_thresh, y, dct_rows, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);
    else
      if (suppChange_flag)
        [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = UnrestrictedCG_M_dct(vec, grad, grad_previous, grad_prev_thresh, y, dct_rows, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);
      else
        [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = RestrictedCGwithSupportEvolution_M_dct(vec, grad, grad_previous, grad_prev_thresh, y, dct_rows, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);    
