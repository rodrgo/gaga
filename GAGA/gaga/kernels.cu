...
__global__ void threshold_four(float *out, const float *in, const float *flag, const int n)
{
...
__global__ void threshold_three(float *out, const float *in, const float *flag, const int n)
{
  unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void threshold_two(float *out, const float *flag, const int n)
{
  unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
...
// Check support set evolution
// This function indicates if the support on the current
// iteration has changed from the previous iteration.
// This is used for example in CGIHT to indicate that one should restart.
void __global__ checkSupportEvolution(int * d_bin, int * d_bin_prev, const int k_bin, const int k_bin_prev, int * suppChange, const int n)
{
  unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
  if (xIndex == 0){
        suppChange[0]=0;
  ...
  if ((xIndex < n) && (suppChange[0]==0)){
	if (d_bin_prev[xIndex] <= k_bin_prev){
		if (d_bin[xIndex] > k_bin){
			atomicAdd(suppChange,1);
		}
	}
  }
}
...
// ************* Used to transpose the gen matrix ***********

// This function takes an m x n column indexed matrix d_A and creates the n x m transpose d_AT.
// A faster traspose kernel is available in the CUDA SDK, but cannot be redistributed 
// and at present only works for matrices of certain sizes.

__global__ void matrix_transpose(float *A, float *AT, const int m, const int n)
{
   unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
   // xIndex is a value from 1 to k from the vector ind
