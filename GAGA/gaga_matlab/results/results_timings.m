function [errors, timings, checkSupport, convRate] = results_timings(vec, vec_input, vecDistribution, residNorm_prev, iter, checkSupport, totaltime, time_sum, time_per_iteration, time_supp_set, ksum, alpha, supp_flag, k, m, n, seed, timeTest, algstr)
% results function
...
month=c(2);
day=c(3);
fname=sprintf('matlab_timings_data_dct%d%02d%02d.txt',year,month,day);

foutput = fopen(fname, 'a+');
if (foutput < 3)
    display('WARNING: Output file did not open!')
else
    File_output_timings(foutput, k, m, n, vecDistribution, errors, timings, iter, convRate, checkSupport, seed, time_per_iteration, time_supp_set, algstr, alpha, supp_flag);
end
fclose(foutput);
