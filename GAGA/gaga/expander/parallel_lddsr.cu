...
inline void parallel_lddsr(float *d_vec, float *d_y, float *d_res, int *d_rows, int *d_cols, float *d_vals, float tol, const int maxiter, const int k, const int m, const int n, const int d, const int nz, float *resRecord, float *timeRecord, int *p_iter, dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksnp, dim3 threadsPerBlocknp, dim3 numBlocksm, dim3 threadsPerBlockm){
	...
	while( (iter < maxiter) & (norm_res > tol) & (norm_res < (100*norm_res_start)) & (residNorm_diff > 0.000001) & (isCycling == 0) & (h_found_candidates == 1)){
		...
		// compute lddsr-scores in parallel

        	cuda_compute_scores_lddsr<<<numBlocks, threadsPerBlock>>>(d_scores, d_res, d_vec, d_updates, d_rows, d_found_candidates, n, d, offset); 
		...
		/*
		// We are not using this strategy in this algorithm. We need to change update in d_updates of cuda_compute_scores_lddsr to use it
		...
		*/
		...
		//printf("iter = %d\toffset = %d\tnorm_Res = %g\n", iter, offset, norm_res);
