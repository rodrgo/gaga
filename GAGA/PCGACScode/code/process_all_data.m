function process_all_data(alg_list,matens_list)
% This function processes data for data without noise and from 
% with binary sparse vectors, i.e. the problem class (Mat, B).
% For data with noise or for other vector distributions with or without noise
% use the script process_all_data_noise.
% Optinal Inputs:
...
if dct == 1
  ...
  if process == 1
    if specific_dates == 1
      process_dct_data(alg_list_dct, startdate_dct, enddate_dct);
    else
      process_dct_data(alg_list_dct);
    end
  end
...
if smv == 1
  ...
  if process == 1
    if specific_dates == 1
      process_smv_data(alg_list_smv, startdate_smv, enddate_smv);
    else
      process_smv_data(alg_list_smv);
    end
  end
...
if gen == 1
  ...
  if process == 1
    if specific_dates == 1
      process_gen_data(alg_list_gen, startdate_gen, enddate_gen);
    else
      process_gen_data(alg_list_gen);
    end
  end
...
add_logit_data_all(alg_list, ens_list)
