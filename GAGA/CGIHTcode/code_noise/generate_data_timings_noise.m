function generate_data_timings_noise(alg_list,matens_list)
...
% if using multiple GPUs, see item below 12 as set the following to 1
usingMultipleGPUs = 0;
...
%n_list=[2^17];  % uncomment this line to generate the data for PCGACS; the script will automatically use n=2^13 for matens=gen 
n_list=2^10;      % this smaller value of n is for quickly generating representative data from PCGACS
...
for nl=1:length(noise_list)
  ...
  if strcmp(alg,'CGIHT')
    ...
    if strcmp(matens,'dct')
      options=gagaOptions('tol',Tol,'maxiter',maxIter,'vecDistribution',vecDistribution,...
                      'noise',noise_level,'gpuNumber',gpuNum,'kFixed','on','projFracTol',6);
    else
      options=gagaOptions('tol',Tol,'maxiter',maxIter,'vecDistribution',vecDistribution,'matrixEnsemble',matrixEnsemble,...
                      'noise',noise_level,'gpuNumber',gpuNum,'kFixed','on','projFracTol',6);
    end
