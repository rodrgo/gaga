function [vec, residNorm_prev, iter, time_sum, ksum] = CGIHT_M_smv(y, A_smv, k, m, n, tol, maxiter, restartFlag)
...
while ( (err>resid_tol) && (iter < maxiter) && (err < 100 * err_start) && (abs(residNorm_diff) > .01*tol) && (fail == 0) )
    ...
    if (restartFlag)
      [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = RestrictedCGwithSupportEvolution_M_smv(vec, grad, grad_previous, grad_prev_thresh, y, A_smv, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);
    else
      if (suppChange_flag)
        [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = UnrestrictedCG_M_smv(vec, grad, grad_previous, grad_prev_thresh, y, A_smv, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);
      else
        [vec, grad, grad_previous, grad_prev_thresh, residNorm_prev, mu, maxChange] = RestrictedCGwithSupportEvolution_M_smv(vec, grad, grad_previous, grad_prev_thresh, y, A_smv, bin, residNorm_prev, kbin, m, n, mu, beta_to_zero_flag);    
