...
inline void parallel_l0_swipe(float *d_vec, float *d_y, float *d_res, int *d_rows, int *d_cols, float *d_vals, float tol, const int maxiter, const int k, const int m, const int n, const int d, const int nz, float *resRecord, float *timeRecord, int *p_iter, dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksnp, dim3 threadsPerBlocknp, dim3 numBlocksm, dim3 threadsPerBlockm){
	...
	while( (iter < maxiter) & (norm_res > tol) & (norm_res < (100*norm_res_start)) & (residNorm_diff > 0.000001) & (isCycling == 0) & (h_found_candidates == 1) & (alpha > 0) ){
		...
		// compute scores and update x (if possible).
		...
		if (h_found_candidates == 0 & alpha > 0){
			alpha--;
			h_found_candidates = 1;
		}else{
			// Update residual if we found candidates
