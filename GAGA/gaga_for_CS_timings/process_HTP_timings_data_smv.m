fnames=ls;
fname_start='gpu_timings_HTP_data_smv';
...
data_ready=1;
for supp_flag=0:0
  fname_save=sprintf('results_HTP_smv_timing_supp_flag_%d.mat',supp_flag);
...
if (data_ready==0)
  ...
  fname=['gpu_timings_HTP_data_smv' date_string '.txt'];
  fid = fopen(fname);
  if fid~=-1
    display(date_string)
    ...
    while(1)
      ...
      while (p<1000)
        ...
        tmp{48}=[tmp2{2} tmp2{4} tmp2{6} tmp2{8} tmp2{10}];
