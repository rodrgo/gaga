fnames=ls;
fname_start='gpu_timings_HTP_data_gen';
...
fname_save=sprintf('results_HTP_gen_timing_supp_flag_%d.mat',supp_flag);
...
if fid~=-1
  ...
  while (str2num(date_string)<=str2num(enddate))
    ...
    fname=['gpu_timings_HTP_data_gen' date_string '.txt'];
    fid = fopen(fname);
    ...
    if fid~=-1
      ...
      while(1)
        ...
        while (p<1000) 
          ...
          tmp{46}=[tmp2{2} tmp2{4} tmp2{6} tmp2{8} tmp2{10}];
