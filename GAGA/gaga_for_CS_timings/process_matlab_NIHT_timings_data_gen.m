fnames=ls;
fname_start='matlab_timings_data_gen';
...
fname_save=sprintf('results_NIHT_gen_matlab_timing_supp_flag_%d.mat',supp_flag);
...
if fid~=-1
  ...
  while (str2num(date_string)<=str2num(enddate))
    ...
    fname=['matlab_timings_data_gen' date_string '.txt'];
    fid = fopen(fname);
...
%46, a 3 by iterations array with the first column being 
...
%as this is eleven entries, we strip results into an 11 column array
    ...
    for ll=1:length(k_grid)
      ...
      if min(size(ind))>0
        ...
        for lll=1:length(ind)
          ...
          time_tmp_per=[time_tmp_per; results_long{ind(lll),46}(:,2)-results_long{ind(lll),46}(:,3)];
  ...
  save(fname_save,'kmn_list','time_per_iteration_kmn_all','time_supp_set_kmn_all','results','results_content');
