fnames=ls;
fname_start='matlab_timings_HTP_data_smv';
...
data_ready=1;
fname_save=sprintf('results_HTP_smv_matlab_timing_supp_flag_%d.mat',supp_flag);
...
if (data_ready==0)
  ...
  fname=['matlab_timings_HTP_data_smv' date_string '.txt'];
  fid = fopen(fname);
  if fid~=-1
    display(date_string)
    ...
    while(1)
      ...
      while (p<1000)
        ...
        tmp{48}=[tmp2{2} tmp2{4} tmp2{6} tmp2{9} tmp2{12}];
...
%as this is eleven entries, we trip results into a 13 column array
  ...
  num_tests=size(results_full,1);
    ...
    time_per_cg_iteration_kmnp_all=cell(0);
    total_cg_steps_kmnp_all=cell(0);
      ...
      for ll=1:length(k_grid)
        ...
        if min(size(ind))>0
          ...
          for lll=1:length(ind)
            ...
            time_tmp_per=[time_tmp_per; results_long{ind(lll),48}(:,2)-results_long{ind(lll),48}(:,3)-results_long{ind(lll),48}(:,5)];
            time_tmp_supp=[time_tmp_supp; results_long{ind(lll),48}(:,3)];
            cg_steps=[cg_steps; sum(results_long{ind(lll),48}(:,4))];
            ind_tmp=find(results_long{ind(lll),48}(:,4)>0);
            time_cg=[time_cg; results_long{ind(lll),48}(ind_tmp,5)./results_long{ind(lll),48}(ind_tmp,4)];
          ...
          a{1}=sort(time_cg);
          time_per_cg_iteration_kmnp_all=[time_per_cg_iteration_kmnp_all a];
          a{1}=sort(cg_steps);
          total_cg_steps_kmnp_all=[total_cg_steps_kmnp_all a];
    ...
    if prod(kmnp_list)>0
      ...
      time_per_cg_iteration_kmnp_all=time_per_cg_iteration_kmnp_all(ind);
      total_cg_steps_kmnp_all=total_cg_steps_kmnp_all(ind);
    end
    
    fname_save=sprintf('results_HTP_smv_matlab_timing_supp_flag_%d.mat',supp_flag);
    save(fname_save,'kmnp_list','time_per_iteration_kmnp_all','time_supp_set_kmnp_all','time_per_cg_iteration_kmnp_all','total_cg_steps_kmnp_all','results','results_content');
  end
