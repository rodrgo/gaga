function [km_list data_ave_list data_sd_list] = make_delta_rho_plot_noise(alg,ens,n,data_type,nonzeros,noise_level,vecDistr)

% to save the plots made by this function, set savePlots=1
savePlots = 0;
...
fname_load=['results_' alg '_S_' ens vec_str noise_string '.mat'];
load(fname_load);
...
elseif strcmp(data_type,'l_two')
  ind_results=6;
  plot_type=1;
  title_txt_f1=[alg ', ' ens ', Average error (Log10) in the l two norm'];
...
% select the values of n (and possibly nonzeros) specified
if nonzeros==0
...
%extract the correct logit fit parameters
if nonzeros==0
...
ind_tmp=find(delta_list > 0.0001);
delta_list=delta_list(ind_tmp);
logit_betas=logit_betas(ind_tmp,:);
logit_deltas=logit_deltas(ind_tmp);


%delta_list=deltas{ind_tmp}(:);
%delta_list=delta_list(delta_list > 0.009);
...
if prod(km_list)>0
  ...
  fname1=['noiseplots/delta_rho_' alg '_' ens vec_str fname_noise_string '_' data_type];
  fname2=[fname1 '_sd_'];
