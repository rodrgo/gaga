...
//  device variables for FIHT_S_gen and ALPS_S_gen
    float * d_vec_prev;
    float * d_Avec_prev;
    float * d_vec_extra;
    float * d_Avec_extra;
//  device variable for CGIHTprojected
...
// allocate memory on the device 
    ...
    cudaMalloc((void**)&d_A, mn * sizeof(float));
    SAFEcudaMalloc("d_A");

    cudaMalloc((void**)&d_AT, mn * sizeof(float));
    SAFEcudaMalloc("d_AT");
    ...
    if (strcmp(algstr, "CGIHT")==0 || strcmp(algstr, "CGIHTprojected")==0){ // CGIHTprojected
...
// allocate memory on the host specific for timing set to on
    ...
    if (nlhs == 6) {
      ...
      h_A = (float*)malloc( sizeof(float) * mn);
    }
    #endif
...
*/
    ...
    if (nlhs == 6){
      if (kFixedFlag==0){
        if (noise_level <= 0){
          createProblem_gen(&k, m, n, vecDistribution, d_vec_input, d_y, d_A, matrixEnsemble, &seed);
          SAFEcuda("createProblem_gen"); }
	else{ //  when noise_level > 0 
	  createProblem_gen_noise(&k, m, n, vecDistribution, d_vec_input, d_y, d_A, matrixEnsemble, &seed, noise_level);
 	  SAFEcuda("createProblem_gen_noise"); }
      }  else{
        int k_tmp = k;
	do{
	  k = k_tmp;
      ...
      cudaMemcpy(d_A,h_A, sizeof(float)*mn, cudaMemcpyHostToDevice);
    }
...
//  ensure initial approximation is the zero vector
  ...
  // Store the transpose matrix
    matrix_transpose<<< numBlocks, threadsPerBlock >>>(d_A, d_AT, m, n);
    SAFEcuda("matrix_transpose in gaga_gen");
...
// Initialization of parameters and cublas
	...
	case 0:
	   if (timingFlag == 0){
		NIHT_S_gen(d_vec, d_vec_thres, grad, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("NIHT_S_gen in gaga_gen");}
	   else {
	   	NIHT_S_timings_gen(d_vec, d_vec_thres, grad, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, alpha, supp_flag, time_per_iteration, time_supp_set, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
		SAFEcuda("NIHT_S_timings_gen in gaga_gen");}
	   break; 
	case 1:
	   if (timingFlag == 0){
		HTP_S_gen(d_vec, d_vec_thres, grad, grad_previous, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("HTP_S_gen in gaga_gen");}
	   else {
	   	HTP_S_timings_gen(d_vec, d_vec_thres, grad, grad_previous, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, alpha, supp_flag, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
		SAFEcuda("HTP_S_timings_gen in gaga_gen");}
	   break;
	case 2:
		mu = 0.65f;
		IHT_S_gen(d_vec, d_vec_thres, grad, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum, numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("IHT_S_gen in gaga_gen");
  		break;
	case 3:
		HT_SD_S_gen(d_vec, d_vec_thres, grad, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("HT_SD_S_gen in gaga_gen");
		break;
	case 4:
		HT_CG_S_gen(d_vec, d_vec_thres, grad, grad_previous, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("HT_CG_S_gen in gaga_gen");
		break;
	case 5:
		CSMPSP_S_gen(d_vec, d_vec_thres, grad, grad_previous, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, d_bin_grad, d_bin_counters_grad, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("CSMPSP_S_gen in gaga_gen");
		break;
	case 6:
		CGIHT_S_gen(d_vec, d_vec_thres, grad, grad_previous, grad_prev_thres, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, d_bin_grad, d_bin_counters_grad, h_bin_counters, residNorm_prev, tol, maxiter, num_bins, k, m, n, restartFlag, &iter, mu, err, &sum, &time_sum,  numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
  		SAFEcuda("CGIHT_S_gen in gaga_gen");
		break;
        case 7: 
                FIHT_S_gen(d_vec, d_vec_thres, grad,  d_vec_prev, d_Avec_prev, d_vec_extra, d_Avec_extra,  d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, 
tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum, numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
                SAFEcuda("FIHT_S_gen in gaga_gen");
                break;
        case 8: 
                ALPS_S_gen(d_vec, d_vec_thres, grad,  d_vec_prev, d_Avec_prev, d_vec_extra, d_Avec_extra,  d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, 
tol, maxiter, num_bins, k, m, n, &iter, mu, err, &sum, &time_sum, numBlocks, threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
                SAFEcuda("ALPS_S_gen in gaga_gen");
                break;
        case 9:
                CGIHTprojected_S_gen(d_vec, d_vec_thres, grad, grad_prev_thres, d_p_thres, d_Ap_thres, d_vec_diff, d_y, resid, resid_update, d_A, d_AT, d_bin, d_bin_counters, h_bin_counters, residNorm_prev, tol, maxiter,  projFracTol,  num_bins,  k,  m, n, &iter,  mu, err, &sum, &time_sum,  numBlocks,  threadsPerBlock, numBlocksm, threadsPerBlockm, numBlocks_bin, threadsPerBlock_bin);
                SAFEcuda("CGIHTprojected_S_gen in gaga gen");
                break;
	default:
		printf("[gaga_gen] Error: The possible (case sensitive) input strings for algorithms using gaga_gen are:\n NIHT\n IHT\n HTP\n ThresholdSD\n ThresholdCG\n CSMPSP\n CGIHT\n FIHT\n");
		break;
    }
...
k = k;
    ...
    if (nlhs == 6){
      if ( ((timingFlag == 0) || ((alg != 0) && (alg != 1)) ) && (noise_level <= 0) ){
        results_gen(d_vec, d_vec_input, vecDistribution, residNorm_prev, h_norms, h_out, h_times, convergence_rate, total_iter, checkSupport, iter, timeIHT, time_sum, &sum, k, m, n, matrixEnsemble, seed,  p_startTest, p_stopTest, algstr, numBlocks, threadsPerBlock);
	SAFEcuda("gaga_gen: results_gen"); }
      else if ( ((timingFlag == 0) || ((alg != 0) && (alg != 1)) ) && (noise_level > 0) ){
        results_gen_noise(d_vec, d_vec_input, vecDistribution, residNorm_prev, h_norms, h_out, h_times, convergence_rate, total_iter, checkSupport, iter, timeIHT, time_sum, &sum, k, m, n, matrixEnsemble, seed, noise_level, p_startTest, p_stopTest, algstr, numBlocks, threadsPerBlock);
 	SAFEcuda("gaga_gen: results_gen_noise"); }
      else if ( (timingFlag == 1) && (alg == 0) ){
         results_timings_gen(d_vec, d_vec_input, vecDistribution, residNorm_prev, h_norms, h_out, h_times, convergence_rate, total_iter, checkSupport, iter, timeIHT, time_sum, time_per_iteration, time_supp_set, &sum, alpha_start, supp_flag, k, m, n, matrixEnsemble, seed,  p_startTest, p_stopTest, algstr, numBlocks, threadsPerBlock);
	SAFEcuda("results_timings_gen in gaga_gen"); }
      else if ( (timingFlag == 1) && (alg == 1) ){
         results_timings_HTP_gen(d_vec, d_vec_input, vecDistribution, residNorm_prev, h_norms, h_out, h_times, convergence_rate, total_iter, checkSupport, iter, timeIHT, time_sum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg, &sum, alpha_start, supp_flag, k, m, n, matrixEnsemble, seed,  p_startTest, p_stopTest, algstr, numBlocks, threadsPerBlock);
	SAFEcuda("results_timings_HTP_gen in gaga_gen"); }
    }
    else if (nlhs == 3){
