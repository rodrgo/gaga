function [errors, timings, iter, checkSupport, convRate, xout] = gaga_matlab_dct(algstr,k, m, n, options)
% This function has the usage 
% [errors, timings, iter, checkSupport, convRate, xout] = gaga_matlab_dct(algstr,k, m, n, options);
...
if valid_alg == 0 
	fprintf('[gaga_matlab_dct Error: The possible (case sensitive) input strings for algorithms using gaga_matlab_dct are: \n NIHT\n IHT\n HTP\n ThresholdSD\n ThresholdCG\n CSMPSP\n CGIHT\n');
else
...
switch alg;
  case 0
    if (timingFlag == 0)
	[vec, residNorm_prev, iter, time_sum, ksum] = NIHT_M_dct(y, dct_rows, k, m, n, tol, maxiter);
    else
        [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set] = NIHT_M_timings_dct(y, dct_rows, k, m, n, tol, maxiter, supp_flag);
    end
  case 1
    if (timingFlag == 0)
	[vec, residNorm_prev, iter, time_sum, ksum] = HTP_M_dct(y, dct_rows, k, m, n, tol, maxiter);
    else
        [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg] = HTP_M_timings_dct(y, dct_rows, k, m, n, tol, maxiter, supp_flag);
    end
  case 2
	[vec, residNorm_prev, iter, time_sum, ksum] = IHT_M_dct(y, dct_rows, k, m, n, tol, maxiter);
  case 3
	[vec, residNorm_prev, iter, time_sum, ksum] = HT_SD_M_dct(y, dct_rows, k, m, n, tol, maxiter);
  case 4
  ...
  case 6
	[vec, residNorm_prev, iter, time_sum, ksum] = CGIHT_M_dct(y, dct_rows, k, m, n, tol, maxiter, restartFlag);
  otherwise fprintf('[gaga_matlab_dct Error: The possible (case sensitive) input strings for algorithms using gaga_matlab_dct are: \n NIHT\n IHT\n HTP\n ThresholdSD\n ThresholdCG\n CSMPSP\n CGIHT\n');
end
...
if (noise_level <= 0)
  if (timingFlag == 1)
    if (alg == 0)
      [errors, timings, checkSupport, convRate] = results_timings(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, time_per_iteration, time_supp_set, ...
        ksum, 0, supp_flag, k, m, n, seed, timeTest, algstr);
    elseif (alg == 1)
      [errors, timings, checkSupport, convRate] = results_timings_HTP(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
    ...
    [errors, timings, checkSupport, convRate] = results_dct(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, ksum, k, m, n, seed, timeTest, algstr);
  end
else
  if (timingFlag == 1)
    display('Error: timings flag only valid if noise is set to zero.');
  else
    [errors, timings, checkSupport, convRate] = results_dct_noise(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, ksum, k, m, n, seed, noise_level, timeTest, algstr);
  end
end
