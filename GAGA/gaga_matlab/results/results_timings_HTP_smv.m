function [errors, timings, checkSupport, convRate] = results_timings_HTP_smv(vec, vec_input, vecDistribution, residNorm_prev, iter, checkSupport, totaltime, time_sum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg, ksum, alpha, supp_flag, k, m, n, p, matrixEnsemble, seed, timeTest, algstr)
% results function
...
month=c(2);
day=c(3);
fname=sprintf('matlab_timings_HTP_data_smv%d%02d%02d.txt',year,month,day);

foutput = fopen(fname, 'a+');
if (foutput < 3)
    display('WARNING: Output file did not open!')
else
    File_output_timings_HTP_smv(foutput, k, m, n, vecDistribution, errors, timings, iter, convRate, checkSupport, seed, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg, algstr, alpha, supp_flag, p, matrixEnsemble);
end
fclose(foutput);
