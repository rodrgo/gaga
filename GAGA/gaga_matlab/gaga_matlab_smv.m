function [errors, timings, iter, checkSupport, convRate, xout] = gaga_matlab_smv(algstr, k, m, n, p, options)
% This function has the usage 
% [errors, timings, iter, checkSupport, convRate, xout] = gaga_matlab_smv(algstr, k, m, n, p, tol, maxiter, vecDistribution, matrixEnsemble);
...
valid_alg=0;
if ( strcmp(algstr, 'NIHT') || strcmp(algstr, 'HTP') || strcmp(algstr, 'IHT') || strcmp(algstr, 'ThresholdSD') || strcmp(algstr, 'ThresholdCG') || strcmp(algstr, 'CSMPSP') || strcmp(algstr, 'CGIHT') || strcmp(algstr, 'SMP') || strcmp(algstr, 'SSMP') || strcmp(algstr, 'ER') || strcmp(algstr, 'serial-l0') || strcmp(algstr, 'parallel-l0') )
  valid_alg = 1;
end


if valid_alg == 0 
	fprintf('[gaga_matlab_smv Error: The possible (case sensitive) input strings for algorithms using gaga_matlab_smv are: \n NIHT\n IHT\n HTP\n ThresholdSD\n ThresholdCG\n CSMPSP\n CGIHT\n SMP\n SSMP\n ER\n serial-l0\n parallel-l0');
else
...
if (nargin == 6) 
  if (iscell(options) == 0)
	...
	if (strcmp(options{i,2},'binary')) matrixEnsemble = 2; 
        elseif (strcmp(options{i,2},'ones')) matrixEnsemble = 1;
        end
	...
	supp_flag = options{i,2};
      elseif (strcmp(options{i,1}, 'noise') == 1)
        noise_level = double(options{i,2});
...
if (noise_level <= 0)
  [vec_input, y, A_smv, seed] = createProblem_smv(k, m, n, vecDistribution, p, matrixEnsemble, seed);
else
  [vec_input, y, A_smv, seed] = createProblem_smv_noise(k, m, n, vecDistribution, p, matrixEnsemble, seed, noise_level);
end
...
elseif (strcmp(algstr, 'SMP')==1) alg = 7;
elseif (strcmp(algstr, 'SSMP')==1) alg = 8;
elseif (strcmp(algstr, 'ER')==1) alg = 9;
elseif (strcmp(algstr, 'serial-l0')==1) alg = 10;
elseif (strcmp(algstr, 'parallel-l0')==1) alg = 11;
end

switch alg;
  case 0
    if (timingFlag == 0)
	[vec, residNorm_prev, iter, time_sum, ksum] = NIHT_M_smv(y, A_smv, k, m, n, tol, maxiter);
    else
        [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set] = NIHT_M_timings_smv(y, A_smv, k, m, n, tol, maxiter, supp_flag);
    end
  case 1
    if (timingFlag == 0)
	[vec, residNorm_prev, iter, time_sum, ksum] = HTP_M_smv(y, A_smv, k, m, n, tol, maxiter);
    else
        [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg] = HTP_M_timings_smv(y, A_smv, k, m, n, tol, maxiter, supp_flag);
    end
  case 2
	[vec, residNorm_prev, iter, time_sum, ksum] = IHT_M_smv(y, A_smv, k, m, n, tol, maxiter);
  case 3
	[vec, residNorm_prev, iter, time_sum, ksum] = HT_SD_M_smv(y, A_smv, k, m, n, tol, maxiter);
  case 4
	[vec, residNorm_prev, iter, time_sum, ksum] = HT_CG_M_smv(y, A_smv, k, m, n, tol, maxiter);
  case 5
	[vec, residNorm_prev, iter, time_sum, ksum] = CSMPSP_M_smv(y, A_smv, k, m, n, tol, maxiter);
  case 6
	[vec, residNorm_prev, iter, time_sum, ksum] = CGIHT_M_smv(y, A_smv, k, m, n, tol, maxiter, restartFlag);
  case 7
	[vec, residNorm_prev, iter, time_sum, ksum] = SMP_M_smv(y, A_smv, A_rows, p, k, m, n, tol, maxiter);
  case 8
	[vec, residNorm_prev, iter, time_sum, ksum] = SSMP_M_smv(y, A_rows, p, k, m, n, tol, maxiter);
  case 9
	[vec, residNorm_prev, iter, time_sum, ksum] = ER_M_smv(y, A_rows, p, k, m, n, tol, maxiter);
  case 10
	[vec, residNorm_prev, iter, time_sum, ksum] = SEQL0_M_smv(y, A_rows, p, k, m, n, tol, maxiter);
  case 11
	[vec, residNorm_prev, iter, time_sum, ksum] = PARL0_M_smv(y, A_rows, p, k, m, n, tol, maxiter);
  otherwise fprintf('[gaga_matlab_smv Error: The possible (case sensitive) input strings for algorithms using gaga_matlab_smv are: \n NIHT\n IHT\n HTP\n ThresholdSD\n ThresholdCG\n CSMPSP\n  CGIHT\n SMP\n SSMP\n ER\n serial-l0\n parallel-l0');
end
...
if (noise_level <= 0)
  if (timingFlag == 1)
    if (alg == 0)
      [errors, timings, checkSupport, convRate] = results_timings_smv(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, time_per_iteration, time_supp_set, ...
        ksum, 0, supp_flag, k, m, n, p, matrixEnsemble, seed, timeTest, algstr);
    elseif (alg == 1)
      [errors, timings, checkSupport, convRate] = results_timings_HTP_smv(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
    ...
    [errors, timings, checkSupport, convRate] = results_smv(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, ksum, k, m, n, p, matrixEnsemble, seed, timeTest, algstr);
  end
else
  if (timingFlag == 1)
    display('Error: timings flag only valid if noise is set to zero.');
  else
    [errors, timings, checkSupport, convRate] = results_smv_noise(vec, vec_input, ...
        vecDistribution, residNorm_prev, iter, checkSupport, ...
        totaltime, time_sum, ksum, k, m, n, p, matrixEnsemble, seed, noise_level, timeTest, algstr);
  end
end
