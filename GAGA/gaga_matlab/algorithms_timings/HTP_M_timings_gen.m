function [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg] = HTP_M_timings_gen(y, A_gen, k, m, n, tol, maxiter, supp_flag)
% This function has the usage 
% [vec, residNorm_prev, iter, time_sum, ksum, time_per_iteration, time_supp_set, cg_per_iteration, time_for_cg] 
%       = HTP_M_timings_gen(y, A_gen, k, m, n, tol, maxiter, supp_flag)
...
%    in each iteration;
%  cg_per_iteration is a vector containing the number of CG iterations
%    per iteration of the algorithm;
%  time_for_cg is a vector of times for completing the CG projection in 
...
cg_per_iteration = zeros(maxiter,1);
time_for_cg = zeros(maxiter,1);
...
while ( (err>resid_tol) && (iter < maxiter) && (err < 100 * err_start) && (residNorm_diff > .01*tol) && (fail == 0) )
    ...
    [vec, grad, resid, residNorm_prev, maxChange] = RestrictedSD_M_gen(vec, grad, y, resid, A_gen, bin, bin_counters, residNorm_prev, num_bins, kbin, m, n);
    ...
    while ( (errCG > tolCG) && (iterCG < maxiterCG) && (residNorm_diffCG > tol2) )
        ...
        [vec, grad, grad_previous, residNorm_prevCG, mu] = RestrictedCG_M_gen(vec, grad, grad_previous, y, A_gen, bin, residNorm_prevCG, kbin, m, n, mu);
