...
// Exactly the same as createProblem_svm_noise, but the noise vector
// is distributed as a Normal(0, noise_level^2*I)
int createProblem_smv_noise_ccs(int *k_pointer, const int m, const int n, const int vecDistribution, float *d_vec_input, float *d_y, int *d_rows, int *d_cols, float *d_vals, const int p, const int ensemble, unsigned int *p_seed, float noise_level)
{
// k_pointer is used rather than k as we may allow k to vary somewhat.
// m and n are the size of the matrix
// vecDistribution: 0 = uniform, 1=random{-1,1}, 2 = Gaussian
