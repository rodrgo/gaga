 ...
 *
 * Licensed under the GAGA License available at gaga4cs.org and included as GAGAMC_license.txt.
 *
...
// I DON'T KNOW HOW WE WANT TO SCALE THESE FACTOR MATRICES, SO ......
/*
  float scale = sqrt((float)m);
  scale = 1/scale;
  cublasSscal(mr,scale,d_rand,1);
  SAFEcublas("cublasSscal in creatDataMatrix");

  scale = sqrt((float)n);
  scale = 1/scale;
  cublasSscal(nr,scale,d_rand+mr,1);
