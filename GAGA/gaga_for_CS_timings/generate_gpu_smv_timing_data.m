  ...
  for ii=1:length(n_list)
    ...
    for jj=1:length(delta_list);
      ...
      if m>=m_minimum
        ...
        % for smv we also have to loop over nonZeroList
        for qq=1:length(nonZeroList)
          ...
          for ll=1:length(k_list)
            ...
            % first test NIHT
            options = gagaOptions('vecDistribution','binary','alpha',alpha,'matrixEnsemble','binary','tol',tol*m/n,'supportFlag',suppFlag,'maxiter',maxiter_niht,'timing','on','gpuNumber',gpuNumber);
        
        
            if n<=n_max_smv
              success=0;
              for p=1:tests_per_k
                [en ct it supset rate xout] = gaga_cs('NIHT', 'smv', k, m, n, nonZeroList(qq), options);
            ...
            end % ends if n<n_max_smv for NIHT
          end % ends k_list loop for NIHT
        end  % ends nonZeroList loop for NIHT
        ...
        for qq=1:length(nonZeroList)
          ...
          for ll=1:length(k_list)
            ...
            % second test HTP
            options = gagaOptions('vecDistribution','binary','alpha',alpha,'matrixEnsemble','binary','tol',tol*m/n,'supportFlag',suppFlag,'maxiter',maxiter_htp,'timing','on','gpuNumber',gpuNumber);
        
            if n<=n_max_smv
              success=0;
              for p=1:tests_per_k
                [en ct it supset rate xout] = gaga_cs('HTP', 'smv', k, m, n, nonZeroList(qq), options);
                success = success+ ( en(end) < (tol*10) );
              end
              if success==0
                break;
            ...
            end % ends if n<n_max_smv for HTP
          end % ends k_list loop for HTP
        end  % ends nonZeroList loop for HTP
      
      end %ends m_minimum
      display(sprintf('smv: m = %d for n = %d finished after %f seconds',m,n,toc))
    end %ends delta_list
  end %ends n_list
