function [errors, timings, checkSupport, convRate] = results_dct_noise(vec, vec_input, vecDistribution, residNorm_prev, iter, checkSupport, totaltime, time_sum, ksum, k, m, n, seed, noise_level, timeTest, algstr)
% results function
...
month=c(2);
day=c(3);
fname=sprintf('matlab_data_dct%d%02d%02d_noise%0.2f.txt',year,month,day,noise_level);

foutput = fopen(fname, 'a+');
if (foutput < 3)
    display('WARNING: Output file did not open!')
else
    File_output(foutput, k, m, n, vecDistribution, errors, timings, iter, convRate, checkSupport, seed, algstr);
end
fclose(foutput);
