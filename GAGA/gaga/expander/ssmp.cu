...
inline void ssmp(float *d_vec, float *d_y, float *resid, float *resid_update, 
	...
	*/
	...
	while( (iter < maxiter) & (err > resid_tol) & (err < (100*err_start)) & (residNorm_diff > 0.01*tol) ){
...
**********************************************************
**			SSMP naive			**
** 	Sequantial Sparse Matching Pursuit naive	**
**********************************************************
*/

inline void ssmp_naive(float *d_vec, float *d_y, float *resid, float *resid_update, 
	...
	*/
	...
	while( (iter < maxiter) & (err > resid_tol) & (err < (100*err_start)) & (residNorm_diff > 0.01*tol) ){
		...
		*/
		...
		cudaUpdateSignal<<<numBlocks, threadsPerBlock>>>(d_vec, h_node, d_medians);
		//cudaMemcpy(d_vec+h_node, d_medians+h_node, sizeof(float), cudaMemcpyDeviceToDevice);
