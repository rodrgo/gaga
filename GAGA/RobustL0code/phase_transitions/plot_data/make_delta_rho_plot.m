function [km_list data_ave_list data_sd_list] = make_delta_rho_plot(alg,ens,n,data_type,show_plot,nonzeros, noise_level)
...
% Get data for particular value of noise_level

noise_levels = zeros(1, length(results_cell));
for ell = 1:length(results_cell)
	noise_levels(ell) = results_cell{ell}.noise_level;
end

ell = find(noise_levels == noise_level);
...
results = results_cell{ell}.results;
...
if true
  ...
  if prod(km_list)>0
    ...
    fname1=[fname1 '_sigma_' sprintf('%1.3f', noise_level)];
    fname2=[fname2 '_sigma_' sprintf('%1.3f', noise_level)];
    if strcmp(ens,'smv')
