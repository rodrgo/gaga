function [errors, timings, checkSupport, convRate] = results_gen_noise(vec, vec_input, vecDistribution, residNorm_prev, iter, checkSupport, totaltime, time_sum, ksum, k, m, n, matrixEnsemble, seed, noise_level, timeTest, algstr)
% results function
...
month=c(2);
day=c(3);
fname=sprintf('matlab_data_gen%d%02d%02d_noise%0.2f.txt',year,month,day,noise_level);

foutput = fopen(fname, 'a+');
if (foutput < 3)
    display('WARNING: Output file did not open!')
else
    File_output_gen(foutput, k, m, n, matrixEnsemble, vecDistribution, errors, timings, iter, convRate, checkSupport, seed, algstr);
end
fclose(foutput);
