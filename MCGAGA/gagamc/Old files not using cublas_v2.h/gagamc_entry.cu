	...
	case 1:
	   ...
	   IHT_MC_S_entry(d_Mat, Grad, Grad_proj, d_Y, d_U, d_S, d_V, d_A, d_y, d_u, d_u_prev, d_v, residNorm_prev, mu, m, n, r, p, mn, maxiter, tol, PSVDmaxiter, PSVDtol, gen, &iter, &time_sum, threadsPerBlockp, numBlocksp, threadsPerBlockm, numBlocksm, threadsPerBlocknr, numBlocksnr, threadsPerBlockmn, numBlocksmn);
  	   SAFEcuda("IHT_MC_S_entry in gagamc_entry");
           break;
	case 2:
	   CGIHT_MC_S_entry(d_Mat, Grad, Grad_proj, Grad_prev, Grad_prev_proj, d_Y, d_U, d_S, d_V, d_A, d_y, d_y_work, d_u, d_u_prev, d_v, residNorm_prev, m, n, r, p, mn, maxiter, tol, PSVDmaxiter, PSVDtol, gen, &iter, &time_sum, threadsPerBlockp, numBlocksp, threadsPerBlockm, numBlocksm, threadsPerBlocknr, numBlocksnr, threadsPerBlockmn, numBlocksmn);
...
// some CPU action is needed before the results
    ...
    if (nlhs == 7){
      ...
      results_entry(d_Mat, Grad, h_Mat_input, h_Mat_out, residNorm_prev, h_norms, h_times, convergence_rate, total_iter, d_A, h_A_out, iter, timeALG, time_sum, m, n, r, p, mn, convRateNum, seed, p_startTest, p_stopTest, algstr);
