...
void PowerIter(float *d_u, float *d_u_prev, float *d_v, float *d_s, float *d_A, float *err, const int m, const int n)
{
  float scale=0.0f;

//  cudaMemcpy(d_u_prev, d_u, sizeof(float)*m, cudaMemcpyDeviceToDevice);

  cublasScopy(m, d_u, 1, d_u_prev, 1);
  SAFEcublas("cublasScopy in PowerIter");

  cublasSgemv('n', m, n, 1, d_A, m, d_v, 1, 0.0f, d_u, 1);    
  SAFEcublas("cublasSgemv in PowerIter");

  scale = cublasSnrm2(m, d_u, 1);
  SAFEcublas("cublasSnrm2 in PowerIter");

  scale = 1/scale;

  cublasSscal(m, scale, d_u, 1);
  SAFEcublas("cublasSscal in PowerIter");

  cublasSgemv('t', m, n, 1, d_A, m, d_u, 1, 0.0f, d_v, 1);    
  SAFEcublas("cublasSgemv (2) in PowerIter");

  *d_s = cublasSnrm2(n, d_v, 1);
  SAFEcublas("cublasSnrm2 (2) in PowerIter");
//printf("\n s=%f",*d_s);
  scale = 1/(float)*d_s;

  cublasSscal(n, scale, d_v, 1);
  SAFEcublas("cublasSscal (2) in PowerIter");

  cublasSaxpy(m, -1, d_u, 1, d_u_prev, 1);
  SAFEcublas("cublasSaxpy in PowerIter");

  *err = cublasSnrm2(m, d_u_prev, 1);
...
void PartialSVD(float *d_U, float *d_S, float *d_V, float *d_Mat, float *d_u, float *d_u_prev, float *d_v, const int m, const int n, const int r, const int maxIter, const float Tol, curandGenerator_t gen, dim3 threadsPerBlockm, dim3 numBlocksm)
{ 
...
//printf("Inside PartialSVD");
  ...
  for ( int i=0; i<r; i++){
    ...
    while ( (err > Tol) && (iter < maxIter) ){
      ...
      PowerIter(d_u, d_u_prev, d_v, &s, d_Mat, &err, m, n);
...
//    cudaMemcpy(d_S+Sshift, &s, sizeof(float), cudaMemcpyHostToDevice);
    ...
    s *= -1.0f;
    cublasSger(m, n, s, d_u, 1, d_v, 1, d_Mat, m);
...
void USVt_product(float *d_Mat, float *d_U, float *d_S, float *d_V,  const int m, const int n, const int r, dim3 threadsPerBlocknr, dim3 numBlocksnr)
{
/*
...
*/
  ...
  cublasSgemm('n', 't', m, n, r, 1.0f, d_U, m, d_V, n, 0.0f, d_Mat, m);
...
void ColumnSpProj(float *d_Out, float *d_Mat, float *d_U, float *d_W,  const int m, const int n, const int r)
{
/*
...
*/
  ...
  // W = U^T * Mat
  cublasSgemm('t', 'n', r, n, m, 1.0f, d_U, m, d_Mat, m, 0.0f, d_W, r);
  SAFEcublas("cublasSgemm in ColumnSpProj.");

  // Mat = U * W = U * (U^T * Mat)
  cublasSgemm('n', 'n', m, n, r, 1.0f, d_U, m, d_W, r, 0.0f, d_Out, m);
...
//printf(" so that alpha = %f\n",alpha);
  ...
  PartialSVD(d_U, d_S, d_V, d_Mat, d_u, d_u_prev, d_v, m, n, r, maxIter, Tol, gen, threadsPerBlockm, numBlocksm);
