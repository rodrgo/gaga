...
alg_list{1}= 'smp';
alg_list{2}= 'ssmp';
alg_list{3}= 'parallel_lddsr';
alg_list{4}= 'er';
alg_list{5}= 'parallel_l0';
alg_list{6}= 'serial_l0';
...
for i=1:length(ens_list)
  ...
  options=gagaOptions('maxiter',2*k,'tol',10^(-5),'noise',0.0,'gpuNumber',0,'vecDistribution','gaussian','kFixed','on');
  ...
  if strcmp(ens_list{i},'smv')    
    ...
    options=gagaOptions('maxiter',2*k,'tol',10^(-5),'noise',0.0,'gpuNumber',0,'kFixed','on');
    for j=1:length(alg_list)
