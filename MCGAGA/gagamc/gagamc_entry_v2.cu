...
//    int   sum   = 0;
    ...
    switch (alg) {
	case 0:
	   NIHT_MC_S_entry(d_Mat, Grad, Grad_proj, d_Y, d_U, d_S, d_V, d_A, d_y, d_u, d_u_prev, d_v, residNorm_prev, m, n, r, p, mn, maxiter, tol, PSVDmaxiter, PSVDtol, gen, &iter, &time_sum, threadsPerBlockp, numBlocksp, threadsPerBlockm, numBlocksm, threadsPerBlocknr, numBlocksnr, threadsPerBlockmn, numBlocksmn, handle);
	   SAFEcuda("NIHT_MC_S_entry in gagamc_entry");
	   break; 
	case 1:
	   mu = 0.65f;
	   IHT_MC_S_entry(d_Mat, Grad, Grad_proj, d_Y, d_U, d_S, d_V, d_A, d_y, d_u, d_u_prev, d_v, residNorm_prev, mu, m, n, r, p, mn, maxiter, tol, PSVDmaxiter, PSVDtol, gen, &iter, &time_sum, threadsPerBlockp, numBlocksp, threadsPerBlockm, numBlocksm, threadsPerBlocknr, numBlocksnr, threadsPerBlockmn, numBlocksmn, handle);
  	   SAFEcuda("IHT_MC_S_entry in gagamc_entry");
           break;
	case 2:
	   CGIHT_MC_S_entry(d_Mat, Grad, Grad_proj, Grad_prev, Grad_prev_proj, d_Y, d_U, d_S, d_V, d_A, d_y, d_y_work, d_u, d_u_prev, d_v, residNorm_prev, m, n, r, p, mn, maxiter, tol, PSVDmaxiter, PSVDtol, gen, &iter, &time_sum, threadsPerBlockp, numBlocksp, threadsPerBlockm, numBlocksm, threadsPerBlocknr, numBlocksnr, threadsPerBlockmn, numBlocksmn, handle);
	   SAFEcuda("CGIHT_MC_S_entry in gagamc_entry");
	   break; 
	default:
...
// free up the allocated memory on the device
    ...
    SAFEcuda("cudaFree in gagamc_entry");
