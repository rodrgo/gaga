fnames=ls;
fname_start='matlab_timings_data_dct';
...
data_ready=1;
fname_save=sprintf('results_NIHT_dct_matlab_timing_supp_flag_%d.mat',supp_flag);
...
if (data_ready==0) 
  ...
  fname=['matlab_timings_data_dct' date_string '.txt'];
  fid = fopen(fname);
...
%as this is eleven entries, we strip results into an 11 column array
  ...
  num_tests=size(results_full,1);
    ...
    %extract the data for NIHT_M_dct with the specified
    ...
    for j=1:length(mn_list);
      ...
      for ll=1:length(k_grid)
        ...
        for lll=1:length(ind)
          ...
          time_tmp_per=[time_tmp_per; results_long{ind(lll),44}(:,2)-results_long{ind(lll),44}(:,3)];
