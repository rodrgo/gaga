...
for ii=1:length(n_list)
  ...
  for jj=1:length(delta_list);
    ...
    if m>=m_minimum
      ...
      options = gagaOptions('vecDistribution','binary','tol',tol*m/n,'supportFlag',0,'maxiter',maxiter_htp,'timing','on');
      
      %first test HTP for dct
      ...
      for ll=1:length(k_list)
        ...
        if n<=n_max_dct
          ...
          for p=1:tests_per_k
            global m;
            global n;
            [en ct it supset rate xout] = gaga_matlab_cs('HTP', 'dct', k, m, n, options);
      ...
      end %end the k_list loop for HTP dct
      
      % second test gen
      options = gagaOptions('vecDistribution','binary','matrixEnsemble','gaussian','tol',tol*m/n,'supportFlag',0,'maxiter',maxiter_htp,'timing','on');
      ...
      for ll=1:length(k_list)
        ...
        if n<=n_max_gen
          ...
          for p=1:tests_per_k
            global m;
            global n;
            [en ct it supset rate xout] = gaga_matlab_cs('HTP', 'gen', k, m, n, options);
            success = success+ ( en(end) < (tol*10) );
          end
          if success==0
            break;
      ...
      end %end the k_list loop for HTP gen
      

      % for smv we test each of the nonzero list
      % third test smv
      options = gagaOptions('vecDistribution','binary','matrixEnsemble','binary','tol',tol*m/n,'supportFlag',0,'maxiter',maxiter_htp,'timing','on');
        ...
        for ll=1:length(k_list)
          ...
          if n<=n_max_smv
            ...
            for p=1:tests_per_k
              global m;
              global n;
              [en ct it supset rate xout] = gaga_matlab_cs('HTP', 'smv', k, m, n, nonZeroList(qq), options);
              success = success+ ( en(end) < (tol*10) );
            end
            if success==0
              break;
    ...
    display(sprintf('HTP matlab: m = %d for n = %d finished after %f seconds',m,n,toc))
  end %ends delta_list
end %ends n_list
