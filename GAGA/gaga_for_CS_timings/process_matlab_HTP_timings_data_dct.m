fnames=ls;
fname_start='matlab_timings_HTP_data_dct';
...
data_ready=1;
fname_save=sprintf('results_HTP_dct_matlab_timing_supp_flag_%d.mat',supp_flag);
...
if (data_ready==0) 
  ...
  fname=['matlab_timings_HTP_data_dct' date_string '.txt'];
  fid = fopen(fname);
  ...
  %stored in each column is listed below.
    ...
    while(1)
      ...
      while (p<1000) 
        ...
        tmp{44}=[tmp2{2} tmp2{4} tmp2{6} tmp2{9} tmp2{12}];
