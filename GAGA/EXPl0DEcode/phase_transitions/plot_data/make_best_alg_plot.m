function [km_list best_alg time_ave_list error_ave_list] = make_best_alg_plot(alg_list, ensemble, n, nonzeros, destination)
...
for i = 1:num_algs
	[junk errs junk] = make_delta_rho_plot(alg_list{i},ensemble,n,'l_infinity',0, nonzeros);
	[km tt junk] = make_delta_rho_plot(alg_list{i},ensemble,n,'time_for_alg',0, nonzeros);
	errors_cell{i} = errs;
	km_cell{i} = km;
	times_cell{i} = tt;
end
...
title(['Algorithm selection map for d = ' num2str(nonzeros) ' with n = 2^{' num2str(log2(n)) '}'],'Fontsize',12)
fname_out=[destination '/algorithm_selection_' ensemble '_n_' num2str(n) '_nonzeros_' num2str(nonzeros)];
...
title(['Time (ms) of fastest algorithm for d = ' num2str(nonzeros) ' with n = 2^{' num2str(log2(n)) '}'],'Fontsize',12)

fname_out=[destination '/best_time_' ensemble '_n_' num2str(n) '_nonzeros_' num2str(nonzeros)];
print('-dpdf', strcat(fname_out, '.pdf'))
print('-depsc', strcat(fname_out, '.eps'))
...
for i = 1:num_algs
	...
	title(['Time (ms) of ' strrep(alg_list{i}, '_', '-') ' / fastest algorithm for d = ' num2str(nonzeros) ' with n = 2^{' num2str(log2(n)) '}'],'Fontsize',12) % fix name

	fname_out=[destination '/' alg_list{i} '_ratio_' ensemble '_n_' num2str(n) '_nonzeros_' num2str(nonzeros)];
	print('-dpdf', strcat(fname_out, '.pdf'))
	print('-depsc', strcat(fname_out, '.eps'))
